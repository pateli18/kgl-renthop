...
def get_data(dataset_filepath, routes_filepath, destination_geocode):
	try:
	...
	while len(ids_to_pull) > 0:
		...
		start_lat = df.iloc[route_id]['latitude']
		start_lng = df.iloc[route_id]['longitude']
		end_lat = df.iloc[route_id][destination_geocode][0]
		end_lng = df.iloc[route_id][destination_geocode][1]
